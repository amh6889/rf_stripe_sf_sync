...
def test_map_open_active_subscription_works(mocked_donor_id, open_subscription_json):
    subscription = json.loads(open_subscription_json)
    mapped_subscription = SubscriptionProcessor._map_subscription(**subscription)
    assert mapped_subscription['Stripe_Subscription_ID__c'] == 'sub_1OuRZCL1MLd6bigCf9ifDf0R'
    assert mapped_subscription['npe03__Amount__c'] == 10000
    assert mapped_subscription['npe03__Date_Established__c'] == '2024-03-15T03:36:22+00:00'
    assert mapped_subscription['npsp__StartDate__c'] == '2024-03-15T03:36:22+00:00'
    ...
    assert mapped_subscription['npsp__Day_of_Month__c'] == 15
