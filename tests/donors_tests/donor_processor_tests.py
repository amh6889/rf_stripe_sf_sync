...
def test_stripe_create_event_handles_salesforce_error_gracefully(donor_with_metadata_address_dict):
    ...
    mocked_salesforce_donor_service = MagicMock()
    mocked_salesforce_donor_service.get_contact_id.return_value = None
    mocked_salesforce_donor_service.create.return_value = {'errors': [{'message': 'mock error message',"errorCode" : "NOT_FOUND"}], 'success': False}

    donor_event_processor = DonorEventProcessor(mocked_donor_mapper, mocked_stripe_donor_service,
                                                mocked_salesforce_donor_service)
    # act
    with pytest.raises(Exception, match="Did not create Stripe customer with bill_nye_test@gmail.com successfully in Salesforce due to: mock error message"):
        created_id = donor_event_processor.process_create_event(donor_with_metadata_address_dict)
        # assert
...
def test_stripe_update_event(donor_with_address_line2_missing):
    ...
    response = donor_event_processor.process_update_event(donor_with_address_line2_missing)
