...
#TODO: I added back external contact id for salesforce field, make sure to test to see it works
class DonorMapper:
    @staticmethod
    def map_donor_update_event(**event_data):
        data = event_data['data']['object']
        customer_id = data.get('id')
        full_name = data.get('name')
        updates = {}
        if not full_name:
            donor_names = DonorMapper.get_donor_name(data)
            first_name = donor_names.get('first_name')
            last_name = donor_names.get('last_name')
            updates['name'] = first_name + ' ' + last_name
        else:
            first_name, last_name = DonorMapper._parse_name(full_name)

        donor_address = DonorMapper.get_donor_address(data)
        ...
        donor = {'FirstName': first_name, 'LastName': last_name, 'npe01__HomeEmail__c': data.get('email'),
                 ...
                 'MailingPostalCode': donor_address.get('postal_code'), 'External_Contact_ID__c': customer_id,
    ...
    @staticmethod
    def map_donor_create_event(**event_data):
        data = event_data['data']['object']
        customer_id = data.get('id')
        full_name = data.get('name')
        updates = {}
        ...
        address = data.get('address')
        donor_address = DonorMapper.get_donor_address(data)
        opt_out = DonorMapper.get_donor_opt_out(data)
        receipt_preference = DonorMapper.get_donor_receipt_preference(data)
        if not address:
            updates['address'] = donor_address

        donor = {'FirstName': first_name, 'LastName': last_name, 'External_Contact_ID__c': customer_id,
