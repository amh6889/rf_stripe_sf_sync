...
class DonationMapper:
    ...
    def map_donation(**event_data):
        ...
        epoch_time_created = data.get('created')
        closed_date = DonationMapper._parse_epoch_time(epoch_time_created)
        status = data.get('status')
        stage_name = DonationMapper._map_stage_name(status)
        payment_method = data['payment_method_details']
        last_4_digits = DonationMapper._get_payment_method_last_4(payment_method)
