...
class DonationMapper:
    ...
    def map_donation(self, event_data):
        ...
        donor_email = None
        if stripe_customer_id := data.get('customer'):
            donor_email = self.stripe_donor.get_donor_email(stripe_customer_id)
        else:
            if billing_details := data.get('billing_details'):
                if email := billing_details.get('email'):
                    donor_email = email
        ...
        donation = {'npe01__Contact_Id_for_Role__c': salesforce_id,
                    ...
                    'npsp__Honoree_Contact__c': honoree_contact_id
                    }
        return donation
